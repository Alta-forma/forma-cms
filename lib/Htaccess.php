...
HTA;
    ...
    public static function uploadsHtaccessContent(): string {
        return <<<'HTA'
...
<IfModule mod_rewrite.c>
    RewriteEngine On
    RewriteRule \.(?i:php[0-9]?|phtml|phar)$ - [F,L]
</IfModule>
HTA;
    }

    /** Drop a deny-PHP .htaccess into uploads/ if missing or still the empty default. */
    public static function ensureUploadsHtaccess(): bool {
        ...
        $path = rtrim(UPLOADS_DIR, '/\\') . '/.htaccess';
        $want = self::uploadsHtaccessContent();
        if (is_file($path)) {
            $have = (string)file_get_contents($path);
            if (str_contains($have, 'uploads must never execute PHP') || trim($have) !== '') {
                return true;
            }
        }
