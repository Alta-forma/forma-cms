...
| Uptime | `GET /up` + `fallback/` | Heartbeat JSON. If `/up` dies but `/fallback/php-ok.json` is 200, PHP/FastCGI is down; Apache still serves last-good `/`. SQLite page cache still needs PHP. |
